    ...
    for w in range(paperWidth + 1):
        for h in range(paperHeight + 1):
            ...
            for cutWidth in range(1, w):
                best = max(best,result[cutWidth][h] + result[w - cutWidth][h])
            for cutHeight in range(1, h):
                best = max(best,result[w][cutHeight] + result[w][h - cutHeight])
            result[w][h] = best
...
"""
print (maxValue([2, 2, 4], [3, 3, 5], [5, 4, 16], 10, 3))
print (maxValue([2, 2, 4], [3, 3, 5], [5, 4, 16], 7, 4))