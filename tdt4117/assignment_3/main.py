...
def remove_punctuation_from_paragraph(paragraph):
    """
    Remove string.punctuation + "\n\r\t" from a specific paragraph. 
    Also change each word to lowercase
    """
    new_paragraph = []
    ...
    for word in paragraph:
        ...
        if new_word == "":
            ...
            new_paragraph.append(new_word.lower().strip())
            new_word == ""
...
# Task 1
def generate_paragraphs(file_path):
...
def generate_dictionary(paragraphs):
    """
    return a dictionary object containing each word in the given paragraphs
    """
...
# Task 2
def generate_bag_of_words(paragraphs,stopwords):
    ...
    return dictionary, [dictionary.doc2bow(doc) for doc in paragraphs] # Task 2.2
...
def main():
    ...
    raw_paragraphs, paragraphs = generate_paragraphs("pg3300.txt") # Read the document generating two paragraphs
    stopwords= generate_stopwords("common-english-words.txt") # Get all stopwords
    dictionary, bags = generate_bag_of_words(paragraphs, stopwords) # generate a dictionary and a bag-of-words from the previously read paragraphs
    tfidf_model, tfidf_corpus, matrix_sim, lsi_model, lsi_corpus, lsi_matrix = retrieve_models(bags, dictionary) # produce tdidf and lsi models
    query(dictionary, tfidf_model,tfidf_corpus, matrix_sim,lsi_model,lsi_matrix,raw_paragraphs) # query the corpus using the different models
